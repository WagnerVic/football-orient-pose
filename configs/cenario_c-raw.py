"""Cenário C-raw — Transfer Learning, SEM nenhuma augmentation (nem flip) — ABLAÇÃO.

Idêntico ao Cenário C (pesos COCO + progressive unfreezing em 3 fases), mas **remove o
RandomFlip horizontal** do pipeline de treino. Comparar C-raw vs C revela o impacto do
flip no transfer learning. Não é célula da matriz 2×2 — é uma ablação complementar.
...
# Pesos COCO PyTorch para transfer learning (load_from na Fase 1).
...
# Pipeline de treino SEM RandomFlip (ablação: nenhuma augmentation).
train_pipeline = [
    dict(type="LoadImage"),
...
param_scheduler = [
    dict(type="LinearLR", begin=0, end=3, start_factor=1e-3,
         by_epoch=True, convert_to_iter_based=True),
    dict(type="CosineAnnealingLR", begin=3, end=15, eta_min=1e-6,
         by_epoch=True, convert_to_iter_based=True),
]
