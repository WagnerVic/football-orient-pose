...
#   EPOCHS=150 BATCH=64 GIT_COMMIT=$(git rev-parse HEAD) bash scripts/training/run_c2.sh
set -eo pipefail
...
python scripts/training/train.py --cenario C2 --epochs "$EPOCHS" --batch-size "$BATCH" \
    --work-dir "$RUN/checkpoints/cenario_C2" 2>&1 | tee "$LOG/train_C2.log"

step "AVALIAÇÃO Cenário C2 — val"
python scripts/evaluation/evaluate.py --checkpoint "$RUN/checkpoints/cenario_C2/best_PCK.pth" \
